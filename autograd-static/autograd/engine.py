...
class Executor(object):
    def __init__(self,eval_node_list):
        """
        :param eval_node_list:list of nodes whose values need to be computed.
    ...
    def run(self,feed_dict:dict):
        ...
        topo_order_list = find_topo_sort(self.eval_node_list)
        for node in topo_order_list:
            if isinstance(node.op,PlaceholderOp):
                continue
            vals = [node_to_val_map[node] for node in node.inputs]
...
def gradients(output_node,node_list):
    node_to_output_grads_list_map = {}

    node_to_output_grads_list_map[output_node] = [oneslike_op(output_node)]

    node_to_output_grad_map = {}

    # Traverse graph in reverse topological order given the output_node that we are taking gradient wrt.
    reverse_topo_order = reversed(find_topo_sort([output_node]))

    for node in reverse_topo_order:
        grad = sum_node_list(node_to_output_grads_list_map[node])
        node_to_output_grad_map[node] = grad
        for i in range(len(node.inputs)):
            node_input = node.inputs[i]
            grads = node.op.gradient(node,grad)
            grads_list = node_to_output_grads_list_map.get(node_input,[])
            grads_list.append(grads[i])
            node_to_output_grads_list_map[node_input] = grads_list
    grad_node_list = [node_to_output_grad_map[node]  for node in node_list]
...
class MulByConstOp(Op):
    def __call__(self,node_A:Node,const_val):
        new_node = Op.__call__(self)
        new_node.const_attr = const_val
        new_node.inputs = [node_A]
        new_node.name = "(%s*%s)" %(node_A.name,str(const_val))
...
class DivByConstOp(Op):
    ...
    def __call__(self,node_A:Node,const_val)->Node:
        new_node = Op.__call__(self)
        new_node.inputs = [node_A]
        new_node.const_attr = const_val
        new_node.name = "%s/%s" %(node_A.name,str(const_val))
